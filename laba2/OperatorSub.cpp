...
void OperatorSub::Operate(AString* pObj) {
	...
	} else { // если тип объектов OctString
		long dest1, dest2;
		sscanf(Value1.c_str(), "%o", &dest1); // переводим изображение первого объекта в числовой тип (long dest1)
		sscanf(Value2.c_str(), "%o", &dest2); // переводим изображение второго объекта в числовой тип (long dest2)
		cout << oct << dest1 - dest2 << dec << "\n"; // выводим результат вычитания в восьмеричной системе счисления
	} 
}
